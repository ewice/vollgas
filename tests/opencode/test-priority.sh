...
# Verifies that skills are resolved with correct priority: project > personal > vollgas
...
# 1. Create in vollgas location (lowest priority)
mkdir -p "$HOME/.config/opencode/vollgas/skills/priority-test"
cat > "$HOME/.config/opencode/vollgas/skills/priority-test/SKILL.md" <<'EOF'
---
name: priority-test
description: Vollgas version of priority test skill
---
# Priority Test Skill (Vollgas Version)

This is the VOLLGAS version of the priority test skill.

PRIORITY_MARKER_VOLLGAS_VERSION
EOF
...
if [ -f "$HOME/.config/opencode/vollgas/skills/priority-test/SKILL.md" ]; then
    echo "  [PASS] Vollgas version exists"
else
    echo "  [FAIL] Vollgas version missing"
    exit 1
fi
...
# Test 2: Test that personal overrides vollgas
echo ""
echo "Test 2: Testing personal > vollgas priority..."
...
if echo "$output" | grep -qi "PRIORITY_MARKER_PERSONAL_VERSION"; then
    echo "  [PASS] Personal version loaded (overrides vollgas)"
elif echo "$output" | grep -qi "PRIORITY_MARKER_VOLLGAS_VERSION"; then
    echo "  [FAIL] Vollgas version loaded instead of personal"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
    echo "  Output snippet:"
    echo "$output" | grep -i "priority\|personal\|vollgas" | head -10
fi

# Test 3: Test that project overrides both personal and vollgas
echo ""
echo "Test 3: Testing project > personal > vollgas priority..."
...
if echo "$output" | grep -qi "PRIORITY_MARKER_PROJECT_VERSION"; then
    ...
    echo "  [FAIL] Vollgas version loaded instead of project"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
    echo "  Output snippet:"
...
# Test 4: Test explicit vollgas: prefix bypasses priority
echo ""
echo "Test 4: Testing vollgas: prefix forces vollgas version..."

cd "$TEST_HOME/test-project"
output=$(timeout 60s opencode run --print-logs "Use the use_skill tool to load vollgas:priority-test specifically. Show me the exact content including any PRIORITY_MARKER text." 2>&1) || {
    exit_code=$?
    if [ $exit_code -eq 124 ]; then
        echo "  [FAIL] OpenCode timed out after 60s"
        exit 1
...
if echo "$output" | grep -qi "PRIORITY_MARKER_VOLLGAS_VERSION"; then
    echo "  [PASS] vollgas: prefix correctly forces vollgas version"
elif echo "$output" | grep -qi "PRIORITY_MARKER_PROJECT_VERSION\|PRIORITY_MARKER_PERSONAL_VERSION"; then
    echo "  [FAIL] vollgas: prefix did not force vollgas version"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
fi
